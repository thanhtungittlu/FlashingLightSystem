from configs import MongoConfig, ApplicationConfig
from pymongo import ASCENDING, DESCENDING, MongoClient
import json
import numbers

if __name__ == "__main__":
    indexes_file = ApplicationConfig.WORKING_DIR + "/resources/configs/indexes.json"
    with open(indexes_file, "r") as f:
        indexes_data = json.load(f)
    client_mongo = MongoClient(MongoConfig.uri)
    lst_index = []
    ...
    for mongo_index in lst_index:
        ...
        try:
            ...
            result = client_mongo.get_database().get_collection(mongo_index.get("table")).create_index(
                mongo_index.get("fields"), **key_index
            )
            print(
